...
#' keeps every value a string. The types of a schema with several are read
#' the same way, in the order they are listed: `c("integer", "string")`
#' reads `42` as the integer, `c("string", "integer")` as the string, and the
#' `"null"` of `c("integer", "null")` has no reading, since the wire has no
#' spelling for `null`. An array is the exception: `?tag=a` is read as an
#' array of one only when the schema's own `type` includes `"array"`, not
#' when an array is one of its branches.
#'
